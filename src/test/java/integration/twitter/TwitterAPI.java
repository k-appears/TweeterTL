...
public class TwitterAPI {
    ...
    public void crossTwitterAPICheck_OfficialAdvancedSearchAPI() throws TwitterException {
        ...
        List<Long> oauthIds = statuses.stream().map(Status::getId).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
        ...
        List<Long> searchIds = tweets.stream().map(tweet -> Long.valueOf(tweet.getId())).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
    ...
    public void crossTwitterAPICheck_OfficialScrollDownAPI() throws TwitterException, IOException {
        ...
        List<Long> scrollDownIds = scrollDownTweets.stream().map(ScrollDownAPI.Tweet::getId).sorted(Comparator.reverseOrder()).collect(Collectors.toList());

        // unofficial API 'Scroll Down' not contains all tweets
        oauthIds.removeAll(scrollDownIds);
        assertThat(oauthIds.size()).isBetween(0, 10);
    }
}
